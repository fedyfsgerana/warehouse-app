<template>
    <aside class="fixed top-0 left-0 z-40 flex flex-col h-screen" :style="{
        ...
        <div class="flex items-center gap-3 px-6 py-5" style="border-bottom: 1px solid var(--color-border);">
            <div class="flex items-center justify-center text-base font-black text-white w-9 h-9 rounded-xl"
                style="background: linear-gradient(135deg, var(--color-primary), var(--color-primary-dark));">
                W
            </div>
            <div>
                <p class="text-sm font-bold leading-tight" style="color: var(--color-text);">Warehouse MS</p>
                <p class="text-xs leading-tight" style="color: var(--color-text-muted);">Management System</p>
            </div>
        </div>

        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            <p class="px-3 mb-2 text-xs font-bold tracking-widest uppercase" style="color: var(--color-text-muted);">
                Menu</p>
            ...
            <router-link v-for="item in menuStore.menus" :key="item.id" :to="item.path"
                ...
                <div class="flex items-center justify-center flex-shrink-0 w-8 h-8 transition-all rounded-lg"
        ...
        <div class="px-3 py-4" style="border-top: 1px solid var(--color-border);">
            <div class="flex items-center gap-3 px-3 py-3 rounded-xl" style="background-color: var(--color-surface-2);">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold leading-tight truncate" style="color: var(--color-text);">{{
